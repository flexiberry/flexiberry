...
<Card class="bg-card/45 border-border/30 overflow-hidden shadow-sm">
  <CardHeader class="bg-muted/40 px-3.5 py-3 border-b border-border/30 flex flex-row items-center justify-between gap-3 space-y-0 select-text">
    <span class="text-xs font-black uppercase text-zinc-500 dark:text-zinc-400">Response</span>
    {#if response.status !== undefined}
      <span class="font-mono text-xs px-2.5 py-0.5 rounded font-black border
  ...
  <CardContent class="p-3.5 space-y-3.5 font-mono">
    ...
    {#if response.body}
      <div class="space-y-1.5">
        ...
        <pre class="bg-muted/50 dark:bg-black/25 p-3 rounded-lg border border-border/15 overflow-x-auto max-h-64 text-sm text-zinc-800 dark:text-zinc-300 select-all custom-scrollbar leading-relaxed">{formatBody(response.body)}</pre>
      </div>
    {/if}
  </CardContent>
</Card>

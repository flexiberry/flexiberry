<script lang="ts">
  ...
  export let selectedStep: { taskIndex: number; stepIndex: number } | null = null;
  ...
  function handleSelectStep(taskIndex: number, stepIndex: number, step: PlanStep) {
    if (!isFullscreen) {
  ...
  <ScrollArea class="flex-grow min-h-0">
    <div class="p-3 space-y-2.5">
      {#each plan as task, ti}
        <div class="bg-card/30 border border-border/20 rounded-xl overflow-hidden shadow-sm">
          <!-- Task Header -->
          <button
            type="button"
          ...
          >
            <div class="flex items-center gap-2 overflow-hidden min-w-0">
              ...
              {:else if task.status === "running"}
                <Loader2 class="w-3.5 h-3.5 animate-spin text-cyan-500 shrink-0" />
              {:else if task.status === "failed"}
                <XCircle class="w-3.5 h-3.5 text-rose-500 shrink-0" />
              {:else}
                <PlayCircle class="w-3.5 h-3.5 text-muted-foreground/60 shrink-0" />
              {/if}
              <span class="truncate {task.status === 'running' ? 'text-cyan-500 font-bold' : task.status === 'completed' ? 'text-zinc-800 dark:text-zinc-200' : 'text-zinc-500 dark:text-zinc-400'}">
          ...
          <!-- Step List -->
          {#if isTaskExpanded(ti)}
            <div transition:slide={{ duration: 150 }} class="px-2.5 pb-2.5 border-t border-border/10 bg-muted/5 space-y-2 pt-2">
              {#each task.steps as step, si}
                <div class="bg-muted/40 dark:bg-black/10 border border-border/15 rounded-lg overflow-hidden">
                  <!-- Step Item Row -->
                  <button
                    type="button"
                    class="w-full text-left group flex items-center justify-between text-[11px] font-medium p-2 transition-colors select-none cursor-pointer rounded-lg focus:outline-none
                      {selectedStep?.taskIndex === ti && selectedStep?.stepIndex === si 
                  ...
                  >
                    <div class="flex items-center gap-2 overflow-hidden min-w-0">
                      ...
                      {:else if step.status === "skipped"}
                        <MinusCircle class="w-3.5 h-3.5 text-amber-500 shrink-0" />
                      {:else}
                        <div class="w-3.5 h-3.5 flex items-center justify-center shrink-0">
                          <div class="w-1.5 h-1.5 rounded-full bg-zinc-600/40"></div>
                        </div>
                      {/if}
                      <span class="font-mono truncate {step.status === 'running' ? 'text-cyan-600 dark:text-cyan-400 font-bold' : step.status === 'completed' ? 'text-zinc-700 dark:text-zinc-300' : 'text-muted-foreground/80'}">
                        {step.targetName}
                      </span>
                    </div>
                    ...
                    <div class="flex items-center gap-1.5 shrink-0">
                      {#if step.duration !== undefined}
                        <span class="text-[9px] font-mono text-muted-foreground/50">{step.duration}ms</span>
                      {/if}
                      {#if step.request || step.response || step.error}
                        <div class="text-[9px] text-primary/80 font-bold group-hover:text-primary transition-colors flex items-center gap-0.5 bg-primary/5 px-1 py-0.5 rounded border border-primary/10">
                          <span>Details</span>
                  ...
                  {#if isStepExpanded(ti, si) && (step.request || step.response || step.error)}
                    <div transition:slide={{ duration: 150 }} class="p-2 border-t border-border/10 bg-muted/60 dark:bg-black/20 space-y-2 text-[10px]">
                      ...
                      <!-- Step Error -->
                      {#if step.error}
                        <ErrorDetailsAlert error={step.error} title="Step Error" size="sm" />
                      {/if}
                    </div>
                  {/if}
                </div>
      ...
      {:else}
        <div class="text-[10px] text-muted-foreground/60 italic text-center py-6">Planning execution plan...</div>
      {/each}
    </div>
  </ScrollArea>
</div>

  ...
  const examples = [
    {
      ...
      description: "Explore the comprehensive features of the Berry scripting language. This script showcases all available nodes, blocks, variable declarations, API definitions, data extraction, and logic assertions.",
      features: [
...
Var @EnvironmentConfig Production Settings
- envName: "production"
- timeout_ms: 5000
- isDebug: false
- multilineNote: \`
  This spans multiple lines
  and is parsed natively.
\`
...
- returnedUserId: response.body.user.id
- loginStatus: response.status
Check
- loginStatus == 200 OR loginStatus == 201
- authToken != null
- returnedUserId != null
...
Check
- response.status == 200
- response.body.username == DefaultUser.username
...
`
    },
    {
      ...
      description: "A complete integration test flow calling the standard Swagger Petstore API. It creates a pet using a POST request, captures its auto-generated ID, and uses it to execute a verification GET request.",
      features: [
...
`
    },
    {
      ...
      description: "Demonstrates advanced security patterns including credential decryption and clean global environment pointers. Perfect for UAT and production testing containing encrypted API secrets.",
      features: [
...
`
  ...
  let activeExample = $derived(examples.find(e => e.id === activeTab) || examples[0]);
  ...
  function highlightLine(line: string) {
    ...
    for (let i = 0; i < words.length; i++) {
      ...
      // 1. Keywords
      if (/^(Var|Api|Url|Header|Body|JSON|Task|Step|Params|Capture|Check|Link|Decrypt)$/.test(trimmedPart)) {
      ...
      // 6. Strings/Numbers
      else if (/^["'\`].*["'\`]$/.test(trimmedPart) || /^\d+$/.test(trimmedPart)) {
...
<svelte:head>
  ...
  <meta name="description" content="Explore beautifully designed API integration tests written in Berry language. Copy templates or run directly in the playground." />
</svelte:head>
...
<div class="min-h-screen bg-gray-950 text-gray-100 font-mono flex flex-col justify-between">
  <div>
    ...
    <section class="relative pt-32 pb-12 overflow-hidden text-center">
      ...
      <!-- Grid overlay -->
      <div class="absolute inset-0 bg-[linear-gradient(rgba(52,211,153,0.02)_1px,transparent_1px),linear-gradient(90deg,rgba(52,211,153,0.02)_1px,transparent_1px)] bg-[size:40px_40px] z-0 pointer-events-none" aria-hidden="true"></div>

      <div class="relative z-10 max-width-1100 mx-auto px-6">
        <span class="inline-flex items-center gap-2 text-[0.7rem] uppercase tracking-wider text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 px-3 py-1 rounded-full mb-4">
          <span class="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-ping"></span>
          Interactive Templates
        </span>
        <h1 class="text-4xl md:text-5xl font-extrabold tracking-tight text-white mb-4">
          Berry Language <span class="bg-gradient-to-r from-emerald-400 to-purple-400 bg-clip-text text-transparent">Showcase</span>
        </h1>
        <p class="text-slate-400 max-w-lg mx-auto leading-relaxed text-sm md:text-base">
    ...
    <section class="max-width-1200 mx-auto px-6 pb-24 relative z-10">
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div class="lg:col-span-4 flex flex-col gap-6">
          ...
          <div class="flex flex-col gap-3 bg-gray-900/50 backdrop-blur-md border border-white/5 rounded-2xl p-4">
            <h3 class="text-xs text-slate-500 uppercase tracking-widest font-bold px-2 mb-1">Select Scenario</h3>
            {#each examples as example}
              <button
                onclick={() => activeTab = example.id}
                class="w-full text-left flex flex-col gap-1.5 px-4 py-3 rounded-xl transition-all duration-200 border {activeTab === example.id ? 'bg-emerald-500/10 border-emerald-500/30 text-white shadow-[0_0_20px_rgba(52,211,153,0.1)]' : 'bg-transparent border-transparent text-slate-400 hover:text-slate-200 hover:bg-white/5'}"
              >
                ...
                <span class="text-[0.7rem] text-slate-500 font-mono tracking-tight">{example.subtitle}</span>
              </button>
            {/each}
          </div>

          <!-- Feature Description Card -->
          <div class="bg-gray-900/50 backdrop-blur-md border border-white/5 rounded-2xl p-6 flex flex-col gap-5">
            <div>
              <h3 class="text-base font-bold text-white mb-2">{activeExample.title}</h3>
              <p class="text-xs text-slate-400 leading-relaxed font-sans">{activeExample.description}</p>
            </div>
            
            <div class="border-t border-white/5 pt-4">
              <h4 class="text-xs font-semibold text-emerald-400 uppercase tracking-wide mb-3">Concepts Showcased</h4>
              <ul class="flex flex-col gap-2.5">
                {#each activeExample.features as feature}
                  <li class="flex items-start gap-2 text-[0.72rem] text-slate-300 leading-normal">
            ...
            >
              ...
              <svg width="15" height="15" viewBox="0 0 15 15" fill="none">
                <path d="M5.5 3L10.5 7.5L5.5 12" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
              </svg>
            </a>
          </div>
        ...
        <div class="lg:col-span-8 bg-[#0d1117] border border-white/5 rounded-2xl overflow-hidden shadow-2xl">
          ...
          <!-- Code Window -->
          <div class="overflow-x-auto p-6 font-mono text-[12.5px] leading-6 max-h-[620px] overflow-y-auto custom-scroll">
            <div class="flex flex-col">
              {#each activeExample.code.split('\n') as line, idx}
                <div class="flex items-start hover:bg-white/[0.02] transition-colors rounded px-1">
                  <!-- Line number column -->
                  <span class="w-8 text-right text-slate-600 text-[10px] pr-4 select-none mt-0.5">{idx + 1}</span>
                  ...
                  <span class="flex-1 whitespace-pre">
                    {#each highlightLine(line) as token}
                      ...
                      {:else}
                        <span style="color: {token.type === 'keyword' ? '#c084fc' : 
                                            token.type === 'method' ? '#f97316' :
                                            token.type === 'pointer' ? '#f472b6' :
                                            token.type === 'id' ? '#34d399' :
                                            token.type === 'operator' ? '#93c5fd' :
                                            token.type === 'value' ? '#fcd34d' :
                                            token.type === 'prop' ? '#93c5fd' :
                                            token.type === 'bullet' ? '#fb923c' : '#e5e7eb'}">{token.text}</span>
                      {/if}
                    {/each}
                  </span>
                </div>

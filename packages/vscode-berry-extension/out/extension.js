...
const runnerWebview_1 = require("./runnerWebview");
let client;
function activate(context) {
    ...
    const runFileDisposable = vscode.commands.registerCommand("berry.runFile", async (uri) => {
        ...
        const panel = vscode.window.createWebviewPanel("berryRunner", `Berry Run: ${fileName}`, vscode.ViewColumn.Two, {
            enableScripts: true,
            retainContextWhenHidden: true,
        });
        // Render the template
        const logoUri = panel.webview.asWebviewUri(vscode.Uri.joinPath(context.extensionUri, "icon.png"));
        panel.webview.html = (0, runnerWebview_1.getWebviewContent)(fileName, logoUri.toString());
        ...
        const runExecution = async () => {
            ...
            const core = new berrycore_1.BerryCore(source, {
            ...
            let dataRows = null;
            core.on(berrycore_1.InterpreterEvent.DataLoaded, (payload) => {
                dataRows = payload.rows;
            });
            core.on(berrycore_1.InterpreterEvent.Start, (payload) => {
            ...
            core.on(berrycore_1.InterpreterEvent.TaskBegin, (payload) => {
            ...
            core.on(berrycore_1.InterpreterEvent.TaskDone, (payload) => {
                if (!isDisposed && activeCore === core) {
                    panel.webview.postMessage({
                        type: "taskDone",
            ...
            core.on(berrycore_1.InterpreterEvent.StepBegin, (payload) => {
            ...
            core.on(berrycore_1.InterpreterEvent.ApiCallBegin, (payload) => {
                if (!isDisposed && activeCore === core) {
                    panel.webview.postMessage({
                        type: "apiCallBegin",
            ...
            core.on(berrycore_1.InterpreterEvent.ApiCallDone, (payload) => {
                if (!isDisposed && activeCore === core) {
                    panel.webview.postMessage({
                        type: "apiCallDone",
                        status: payload.status,
            ...
            core.on(berrycore_1.InterpreterEvent.StepDone, (payload) => {
                if (!isDisposed && activeCore === core) {
                    panel.webview.postMessage({
                        type: "stepDone",
                        taskIndex: payload.taskIndex,
            ...
            core.on(berrycore_1.InterpreterEvent.Log, (payload) => {
                if (!isDisposed && activeCore === core) {
                    panel.webview.postMessage({
                        type: "log",
                        level: payload.level,
            ...
            core.on(berrycore_1.InterpreterEvent.Error, (payload) => {
                if (!isDisposed && activeCore === core) {
                    panel.webview.postMessage({
                        type: "error",
                        error: payload,
    ...
    const runApiAloneDisposable = vscode.commands.registerCommand("berry.runApiAlone", async (uri, apiName) => {
        const filePath = uri.fsPath;
        const fileName = path.basename(filePath);
        // Create and show Webview Panel
        const panel = vscode.window.createWebviewPanel("berryRunner", `Berry API: #${apiName}`, vscode.ViewColumn.Two, {
            enableScripts: true,
            retainContextWhenHidden: true,
        });
        // Render the template
    ...
    const codeLensProviderDisposable = vscode.languages.registerCodeLensProvider({ scheme: "file", language: "berry" }, new BerryCodeLensProvider());
    ...
    if (typeof vscode.tests !== "undefined") {
        ...
        const runProfile = controller.createRunProfile("Run API Standalone", vscode.TestRunProfileKind.Run, async (request, token) => {
        ...
        context.subscriptions.push(vscode.workspace.onDidOpenTextDocument(discoverApis), vscode.workspace.onDidChangeTextDocument((e) => discoverApis(e.document)), vscode.workspace.onDidCloseTextDocument((doc) => {

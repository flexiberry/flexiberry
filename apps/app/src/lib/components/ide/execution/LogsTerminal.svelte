...
<div
  class="shrink-0 flex flex-col min-h-0 bg-black/30 {isFullscreen
    ? 'w-full md:w-80 lg:w-[26rem] h-auto'
    : 'w-full h-36'}"
>
  <!-- Realtime Task Status Box (Only in Floating view) -->
  {#if !isFullscreen && status === "running" && (currentTask || currentStep)}
    <div
      class="px-4 py-1.5 border-b border-border/50 bg-black/20 text-[9px] space-y-0.5 shrink-0 flex items-center justify-between gap-4"
    >
      <span class="truncate text-zinc-400 flex items-center gap-1.5">
        <Activity class="w-3 h-3 animate-pulse text-cyan-500 shrink-0" />
        {#if currentTask}Task: <span class="font-bold text-zinc-300"
            >{currentTask}</span
          >{/if}
        {#if currentStep}
          • Step: <code class="text-cyan-500">{currentStep}</code>{/if}
      </span>
    </div>
  {/if}
  ...
  <div class="flex-grow flex flex-col p-3 min-h-0 min-w-0 h-full">
    <div
      class="text-[8px] font-black text-muted-foreground uppercase tracking-widest mb-1.5 shrink-0 select-none"
    >
    ...
    >
      {#each logs as log}
        <div class="flex items-start gap-1.5 leading-relaxed">
          <span class="text-zinc-600 select-none shrink-0 font-light"
            >{log.time}</span
          >
          <span
            class="select-text break-all {log.level === 'error'
      ...
      {:else}
        <div
          class="text-zinc-500 italic select-none h-full flex items-center justify-center"
        >

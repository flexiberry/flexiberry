...
Task Secure Endpoint Verification
  ...
  Step Call Api validateSecureGateway
    ...
    Capture
    - parsedKey: $.headers.X-Api-Key
    - receivedSender: $.json.sender
    Check
    - $.status == 200
`,
  };
  ...
  onMount(async () => {
    ...
    try {
      ...
      addLog(
        "error",
        "Failed to load @flexiberry/berrycore. Running in sandbox simulated mode.",
      );
    }
  });
  ...
  async function runScript() {
    ...
    addLog(
      "system",
      `Starting execution in ${executionMode.toUpperCase()} mode...`,
    );
    ...
    try {
      ...
      coreInstance.on(InterpreterEventEnum.Start, (payload: any) => {
        addLog(
          "info",
          `🚀 Executing Script. Plan: ${payload.totalTasks} Tasks, ${payload.totalApis} APIs registered.`,
        );
      });
      ...
      coreInstance.on(InterpreterEventEnum.ApiCallBegin, (payload: any) => {
        addLog(
          "api",
          `⚡ Calling API #${payload.apiName}: ${payload.method} ${payload.url}`,
        );
      });
      ...
      coreInstance.on(InterpreterEventEnum.ApiCallDone, (payload: any) => {
        addLog(
          "api",
          `   └─ API #${payload.apiName} returned status ${payload.status} (${payload.duration}ms)`,
        );
      });
      ...
      coreInstance.on(InterpreterEventEnum.TaskBegin, (payload: any) => {
        addLog(
          "task",
          `━━━━━━━━ Task [${payload.index + 1}] "${payload.title || "(unnamed)"}" beginning ━━━━━━━━`,
        );
      });
      ...
      coreInstance.on(InterpreterEventEnum.TaskDone, (payload: any) => {
        ...
        const colorType =
          payload.status === ExecutionStatusEnum.Pass ? "system" : "error";
        addLog(
          colorType,
          `${icon} Task Done: "${payload.title}" Status: ${payload.status}`,
        );
      });
      ...
      coreInstance.on(InterpreterEventEnum.StepDone, (payload: any) => {
        const icon = payload.status === ExecutionStatusEnum.Pass ? "✓" : "✗";
        const level =
          payload.status === ExecutionStatusEnum.Pass ? "info" : "warn";
        addLog(
          level,
          `   ${icon} Step [${payload.targetName}] Completed: ${payload.status}`,
        );
      });
      ...
      const passedCount = taskResults.filter(
        (t) => t.status === ExecutionStatusEnum.Pass,
      ).length;
      const failedCount = taskResults.filter(
        (t) => t.status !== ExecutionStatusEnum.Pass,
      ).length;
      ...
      addLog(
        "system",
        `Execution finished in ${elapsed}ms. Passed: ${passedCount}/${taskResults.length}`,
      );
  ...
  function simulateExecution() {
    setTimeout(() => {
      ...
      setTimeout(() => {
        addLog(
          "task",
          "━━━━━━━━ Task [1] 'Pet store sequence' beginning ━━━━━━━━",
        );
        setTimeout(() => {
          addLog(
            "api",
            "⚡ Calling API #addPet: POST https://petstore.swagger.io/v2/pet",
          );
          setTimeout(() => {
            ...
            setTimeout(() => {
              addLog(
                "api",
                "⚡ Calling API #getPetById: GET https://petstore.swagger.io/v2/pet/123456789",
              );
              setTimeout(() => {
                addLog(
                  "api",
                  "   └─ API #getPetById returned status 200 (95ms)",
                );
                    ...
                    steps: [
                      {
                        ...
                        response: {
                          status: 200,
                          headers: {},
                          body: { id: 123456789, name: "BerryDoggie" },
                        },
                      },
                      {
                        targetName: "getPetById",
                        status: "PASS",
                        error: null,
                        checksPassed: true,
                        response: {
...
<svelte:head>
  ...
  <meta
    name="description"
    content="Type, format, and execute Berry API scripts in real-time. View logs and assertion outcomes directly in your web browser."
  />
</svelte:head>
  ...
  <div>
    ...
    <section class="pt-24 pb-12 px-6 max-w-1200 mx-auto w-full">
        ...
        <div class="flex items-center gap-3 border-l border-white/5 pl-4">
          ...
          <div class="flex bg-gray-950 border border-white/10 rounded-lg p-0.5">
            <button
              ...
              class="px-3 py-1 text-[10px] uppercase font-bold rounded-md transition {executionMode ===
              'dry'
                ? 'bg-emerald-500 text-gray-950 shadow-[0_0_10px_rgba(52,211,153,0.3)]'
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
        <!-- Left: Code Editor -->
        <div
          class="lg:col-span-6 flex flex-col bg-[#0d1117] border border-white/5 rounded-2xl overflow-hidden shadow-2xl min-h-[500px]"
        >
          <!-- Editor Title -->
          <div
        ...
        <div
          class="lg:col-span-6 flex flex-col bg-gray-900/40 border border-white/5 rounded-2xl overflow-hidden shadow-2xl"
        >
          <!-- Terminal Tabs -->
          <div class="flex bg-gray-950/80 border-b border-white/5">
            <button
              onclick={() => (activeTabRight = "terminal")}
              class="px-5 py-3 text-xs uppercase font-bold tracking-wider border-b-2 transition {activeTabRight ===
              'terminal'
            ...
            <button
              ...
              class="px-5 py-3 text-xs uppercase font-bold tracking-wider border-b-2 transition relative {activeTabRight ===
              'results'
                ? 'border-emerald-500 text-white bg-white/[0.02]'
                : 'border-transparent text-slate-500 hover:text-slate-300'}"
            >
              ...
              {#if taskResults.length > 0}
                <span
                  class="absolute right-2 top-2.5 w-1.5 h-1.5 rounded-full bg-emerald-400 animate-ping"
                ></span>
              {/if}
            </button>
          </div>
          ...
          {#if activeTabRight === "terminal"}
            ...
            >
              {#if logs.length === 0}
                <div
                  class="text-slate-600 italic select-none py-10 text-center"
                >
              ...
              {:else}
                {#each logs as log (log.id)}
                  <div
                    class="flex items-start gap-3 hover:bg-white/[0.01] rounded py-0.5 px-1"
                  >
                    <span class="text-[10px] text-slate-600 select-none mt-0.5"
                      >{log.timestamp}</span
                    >
                    <span
                      class="flex-1 whitespace-pre-wrap select-text {log.type ===
                      'error'
                        ...
                        : log.type === 'warn'
                          ...
                          : log.type === 'system'
                            ...
                            : log.type === 'task'
                              ...
                              : log.type === 'api'
                                ? 'text-emerald-400/80'
                                : 'text-slate-300'}">{log.message}</span
                    >
                  </div>
                {/each}
              {/if}
          ...
          {:else}
            ...
            <div
              class="flex-1 p-6 bg-gray-950/30 flex flex-col gap-6 overflow-y-auto max-h-[500px] min-h-[420px] custom-scroll"
            >
              {#if taskResults.length === 0}
                <div
                  class="text-slate-600 italic select-none py-12 text-center"
                >
                  No visual execution results yet. Run a script successfully to
                  view metric outcomes.
                </div>
              {:else}
                <!-- Grid metrics -->
                <div class="grid grid-cols-4 gap-4">
                  <div
                    class="bg-gray-950/50 border border-white/5 rounded-xl p-3 text-center"
                  >
                    <span
                      class="block text-[10px] text-slate-500 uppercase font-bold mb-1"
                      >Total Tasks</span
                    >
                    <span class="text-lg font-extrabold text-white"
                      >{executionSummary.total}</span
                    >
                  </div>
                  <div
                    class="bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-3 text-center"
                  >
                    <span
                      class="block text-[10px] text-emerald-400 uppercase font-bold mb-1"
                      >Passed</span
                    >
                    <span class="text-lg font-extrabold text-emerald-400"
                      >{executionSummary.passed}</span
                    >
                  </div>
                  <div
                  ...
                  >
                    <span
                      class="block text-[10px] text-red-400 uppercase font-bold mb-1"
                      >Failed</span
                    >
                    <span class="text-lg font-extrabold text-red-400"
                      >{executionSummary.failed}</span
                    >
                  </div>
                  <div
                    class="bg-purple-500/10 border border-purple-500/20 rounded-xl p-3 text-center"
                  >
                    <span
                      class="block text-[10px] text-purple-400 uppercase font-bold mb-1"
                      >Duration</span
                    >
                    <span
                      class="text-xs font-extrabold text-purple-300 mt-1.5 block"
                      >{executionSummary.duration}ms</span
                    >
                  </div>
                </div>
                ...
                <div class="flex flex-col gap-4">
                  <h4
                    class="text-xs font-bold uppercase tracking-wider text-slate-400 px-1 border-b border-white/5 pb-2"
                  >
                    Scenarios Detail
                  </h4>
                  ...
                  {#each taskResults as task}
                    <div
                      class="bg-gray-950/40 border border-white/5 rounded-xl p-4 flex flex-col gap-3"
                    >
                      <!-- Task main info -->
                      <div class="flex items-center justify-between">
                        <span class="font-bold text-sm text-white"
                          >Task: "{task.title || "(unnamed)"}"</span
                        >
                        <span
                          class="text-[10px] font-bold px-2 py-0.5 rounded-full uppercase {task.status ===
                          'PASS'
                      ...
                      >
                        {#each task.steps as step}
                          <div
                            class="flex flex-col gap-1.5 bg-black/20 p-2.5 rounded-lg border border-white/[0.02]"
                          >
                            <div
                              class="flex items-center justify-between text-xs"
                            >
                              <span class="text-slate-300 font-mono"
                                >Step: <span class="text-emerald-400"
                                  >Call Api {step.targetName}</span
                                ></span
                              >
                              <span
                                class="font-bold text-[9px] uppercase {step.status ===
                                'PASS'
                                  ? 'text-emerald-400'
                                  : 'text-amber-400'}">{step.status}</span
                              >
                            </div>

                            {#if step.response}
                              <div
                                class="flex items-center gap-3 text-[10px] text-slate-500 font-mono bg-[#0d1117]/60 px-2 py-1 rounded"
                              >
                                <span
                                  >Status: <strong class="text-white"
                                    >{step.response.status}</strong
                                  ></span
                                >
                                {#if step.response.body}
                                  <span class="truncate"
                                    >Body preview: <code
                                      class="text-emerald-400/90 font-mono"
                                      >{JSON.stringify(
                                        step.response.body,
                                      ).substring(0, 40)}...</code
                                    ></span

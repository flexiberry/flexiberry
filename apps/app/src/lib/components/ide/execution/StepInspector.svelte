            ...
            <div class="bg-card border border-border/60 rounded-xl overflow-hidden shadow-sm">
              ...
              >
                <div class="flex items-center gap-2">
                  ...
                  <span class="text-[9px] font-black uppercase tracking-wider text-muted-foreground">API Request & Response Details</span>
                </div>
              ...
              {#if apiDetailsExpanded}
                <div transition:slide={{ duration: 150 }} class="p-3 space-y-3 bg-muted/5">
                  {#if step.request}
            ...
            <div class="bg-card border border-border/60 rounded-xl overflow-hidden shadow-sm">
              ...
              >
                <div class="flex items-center gap-2">
                  ...
                  <span class="text-[9px] font-black uppercase tracking-wider text-muted-foreground">Validation Checks ({step.checks.filter(c => c.pass).length}/{step.checks.length})</span>
                </div>
                <div class="text-muted-foreground/60">
                  {#if checksExpanded}
                    <ChevronDown class="w-3.5 h-3.5" />
              ...
              {#if checksExpanded}
                <div transition:slide={{ duration: 150 }} class="p-3 bg-muted/5 divide-y divide-border/10">
                  {#each step.checks as check}
                    <div class="flex items-start justify-between gap-4 py-2 font-mono text-[10px] last:pb-0 first:pt-0">
                      <div class="flex items-center gap-2">
                        {#if check.pass}
                        ...
                        {/if}
                        <span class={check.pass ? 'text-zinc-300' : 'text-rose-400 font-bold'}>{check.expression}</span>
                      </div>
                      <span class="text-muted-foreground/80 select-text break-all text-right font-sans text-[9px]">
                        Evaluated: <code class="bg-muted px-1 py-0.5 rounded font-mono text-[9px] text-zinc-300">{check.evaluated}</code>
                      </span>
                    </div>
                  {/each}
                </div>
            ...
            <div class="bg-card border border-border/60 rounded-xl overflow-hidden shadow-sm">
              ...
              >
                <div class="flex items-center gap-2">
                  ...
                  <span class="text-[9px] font-black uppercase tracking-wider text-muted-foreground">Captured Variables</span>
                </div>
                <div class="text-muted-foreground/60">
                  {#if capturesExpanded}
                    <ChevronDown class="w-3.5 h-3.5" />
                ...
                <div transition:slide={{ duration: 150 }} class="p-3 bg-muted/5 divide-y divide-border/10">
                  {#each Object.entries(step.captures) as [key, value]}
                    <div class="flex items-start justify-between gap-4 py-2 font-mono text-[10px] last:pb-0 first:pt-0">
                      ...
                      <span class="text-zinc-300 select-text break-all text-right">

<script lang="ts">
  ...
  $: {
    // Parse Link path from content by finding the active "Link <path>" line
    const lines = content.split("\n");
    const linkLine = lines.find(line => line.trim().toLowerCase().startsWith("link "));
    if (linkLine) {
      const match = linkLine.trim().match(/^Link\s+(.+)$/i);
  ...
  async function loadWorkspaceData() {
    try {
      ...
      const allFiles = await db.fileStore
        ...
        .equals(workspaceId)
        .toArray();
      // Filter out self and keep only .berry files
      files = allFiles.filter(f => 
        f.name !== currentFileName && 
        f.name.toLowerCase().endsWith('.berry')
      );
  ...
  function updatePath(newPath: string) {
    const lines = content.split("\n");
    const linkLineIndex = lines.findIndex(line => line.trim().toLowerCase().startsWith("link "));
    if (linkLineIndex !== -1) {
      lines[linkLineIndex] = `Link ${newPath}`;
      onChange(lines.join("\n"));
    } else {
      onChange(content ? `${content}\nLink ${newPath}` : `Link ${newPath}`);
    }
  }
...
<div class="flex flex-col gap-2 p-1.5 w-full select-none">
  ...
  <div class="flex items-center gap-2">
    ...
    <div class="relative flex-grow">
      <input
        ...
        placeholder="Type file path (e.g. auth.berry) or remote URL..."
  ...
  {#if pathValue}
    <div class="flex items-center gap-1.5 px-1 text-[10px] font-semibold {isValidPath ? 'text-emerald-500' : 'text-amber-500/80'}">
      ...
      <span>{isValidPath ? 'Valid link target' : 'File does not exist in workspace'}</span>
    </div>
  {/if}
</div>
      ...
      <div class="p-4 border-b border-border/40 flex items-center justify-between bg-muted/20">
        <div class="flex items-center gap-2.5">
          ...
          <div class="flex flex-col">
            <span class="text-sm font-bold text-foreground">Select File to Link</span>
            <span class="text-[10px] text-muted-foreground">Choose from workspace files</span>
          </div>
        </div>
        <button
          type="button"
                ...
                >
                  <div class="flex items-center gap-2">
                    {#if expandedFolders[folder.id]}
                      ...
                      <FolderOpen class="w-4 h-4 text-primary" fill="currentColor" fill-opacity="0.1" />
                    {:else}
                      <ChevronRight class="w-3.5 h-3.5 text-muted-foreground/60" />
                      <Folder class="w-4 h-4 text-primary" fill="currentColor" fill-opacity="0.1" />
                    {/if}
                    ...
                    {#if folder.files.length > 0}
                      {#each folder.files as file}
                        <button
                          ...
                          class="w-full text-left py-1.5 px-2.5 rounded-lg text-xs hover:bg-primary/10 hover:text-primary transition-all flex items-center justify-between group cursor-pointer"
                        >
                          <div class="flex items-center gap-2 truncate">
                            <FileText class="w-3.5 h-3.5 text-muted-foreground/50 shrink-0" />
                            <span class="truncate font-mono">{file.name}</span>
                          </div>
                          {#if pathValue === `${folder.name}/${file.name}`}
                            <span class="text-[9px] font-bold text-pink-500 bg-pink-500/10 px-1.5 py-0.5 rounded border border-pink-500/20">Selected</span>
                          {/if}
                        </button>
                      {/each}
                    {:else}

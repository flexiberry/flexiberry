...
>
    <!-- Header -->
    <div
        class="px-5 py-3 flex items-center gap-3 text-white bg-amber-500/10 border-b border-white/5"
    >
        <div
            class="w-6 h-6 rounded-lg bg-amber-500/20 flex items-center justify-center border border-amber-500/30"
        >
            <CheckCircle2 class="w-3.5 h-3.5 text-amber-400 shrink-0" />
        </div>
        <span
            class="text-[10px] font-black uppercase tracking-[0.2em] text-amber-200 flex-1"
            >{data.title || "Checks"}</span
        >
        <Plus
            class="w-3.5 h-3.5 text-amber-400/50 cursor-pointer hover:text-amber-400 transition-colors"
            on:click={addCheck}
        />
    </div>
        ...
        {#each data.checks as check, i}
            ...
            >
                <div class="flex items-center gap-2">
                    <Input
                        bind:value={check.left}
                        placeholder="Operand"
                        class="h-7 text-[11px] bg-black/20 border-none px-2 font-mono text-amber-200 flex-1 focus-visible:ring-0"
                    />

                    <div
                        class="px-1.5 py-0.5 bg-black/40 rounded border border-white/5"
                    >
                        <select
                            bind:value={check.op}
                    ...
                    <Input
                        bind:value={check.right}
                        placeholder="Expected"
                        class="h-7 text-[11px] bg-black/20 border-none px-2 font-mono text-gray-300 flex-1 focus-visible:ring-0"
                    />
                </div>

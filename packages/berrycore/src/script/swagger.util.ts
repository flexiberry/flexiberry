  ...
  static convertSwaggerToBerry(swagger: string, id: string): string {
    ...
    for (const [path, methods] of Object.entries(obj.paths)) {
      ...
      for (const [method, details] of Object.entries<any>(methods)) {
        ...
        // --- Collect path/query params and update URL ---
        let modPath = path;
        ...
        // --- Build base URL with host/scheme/basePath for Swagger 2, or servers for OpenAPI 3 ---
        let url = modPath;
        if (servers) {
          url = servers + modPath;
        } else if (obj.host) {
        ...
        result += `Api ${method.toUpperCase()} #${opId}\n`;
        result += `Url ${url}\n`;
        // Headers
        let headers: string[] = [];
        if (details.consumes) {
        ...
        if (!bodyHandled && details.parameters) {
          for (const param of details.parameters) {
            if (param.in === "body" && param.schema) {
              bodyExample = SwaggerUtil.generateDummyFromSchema(
                param.schema,
                obj.definitions || obj.components?.schemas
              );
              result += `Body JSON \`${typeof bodyExample === "string" ? bodyExample : JSON.stringify(bodyExample, null, 2)}\`\n`;
              break;
            }
          }
        }

...
{#if $authLoading}
  ...
  <div class="fixed inset-0 z-50 flex flex-col items-center justify-center bg-background">
    <div class="absolute inset-0 z-0 pointer-events-none overflow-hidden bg-background">
      <div class="absolute top-[-10%] left-[-10%] w-[50%] h-[50%] bg-primary/10 rounded-full blur-[100px] animate-pulse"></div>
      <div class="absolute bottom-[-20%] left-[20%] w-[60%] h-[50%] bg-violet-500/10 rounded-full blur-[120px] animate-pulse"></div>
    </div>
...
{:else if !$user}
  ...
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-background/40 backdrop-blur-md overflow-y-auto p-4">
    ...
    <div class="absolute inset-0 z-0 pointer-events-none overflow-hidden bg-background">
      ...
      <div class="absolute inset-0 opacity-20" style="background-image: radial-gradient(hsl(var(--muted-foreground)/0.2) 1px, transparent 1px); background-size: 10px 10px;"></div>
    </div>

    <!-- Auth Card Container -->
    <div class="relative z-10 w-full max-w-md bg-card/80 dark:bg-card/70 border border-border/60 shadow-2xl rounded-2xl p-8 backdrop-blur-xl transition-all duration-300">
      
      <!-- Brand & Header -->
      <div class="mb-8 flex flex-col items-center text-center">
        <div class="w-16 h-16 p-2.5 bg-primary/10 border border-primary/25 rounded-2xl flex items-center justify-center text-primary hover:bg-primary/20 transition-all duration-300 shadow-sm mb-4">
          <img src={berry} class="h-10 w-10 object-contain hover:scale-110 transition-transform duration-200" alt="FlexiBerry Logo" />
        </div>
      ...
      {#if errorMsg}
        <div class="mb-5 rounded-lg border border-destructive/20 bg-destructive/10 p-3 text-xs text-destructive flex items-center gap-2">
      ...
      {#if !isResettingPassword}
        ...
        <div class="grid grid-cols-2 gap-3 mb-5">
          ...
          <Button 
            ...
            disabled={submitting}
          >
            <svg class="h-4 w-4 fill-current text-foreground" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
        ...
        <div class="relative flex py-3 items-center">
          ...
          <span class="flex-shrink mx-4 text-muted-foreground text-[10px] uppercase font-bold tracking-widest">or email & password</span>
          <div class="flex-grow border-t border-border/60"></div>
        </div>
      {/if}

      <!-- Login / Signup Form -->
      <form on:submit={isResettingPassword ? handleEmailAuth : handleEmailAuth} class="space-y-4">
        <div class="space-y-1.5">
          <label for="email" class="text-xs font-semibold text-muted-foreground pl-0.5">Email Address</label>
          <div class="relative">
            <span class="absolute inset-y-0 left-0 flex items-center pl-3 text-muted-foreground/60">
        ...
        <Button type="submit" class="w-full h-11 text-xs font-semibold rounded-xl mt-2 flex items-center justify-center gap-2" disabled={submitting}>
          {#if submitting}
      ...
      <div class="mt-6 flex flex-col items-center gap-3.5 text-center text-xs">
        {#if isResettingPassword}
          <button 
            ...
            on:click={() => { isResettingPassword = false; errorMsg = ""; }}
            disabled={submitting}
          >
            <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        ...
        {:else}
          <button 
            ...
            on:click={() => { isLogin = !isLogin; errorMsg = ""; }}
            disabled={submitting}
          >
          ...
          <div class="w-full flex items-center py-1.5">
            ...
            <span class="flex-shrink mx-3 text-muted-foreground text-[9px] font-bold uppercase tracking-wider">or try it out</span>
            <div class="flex-grow border-t border-border/40"></div>
          </div>

          <button 
            class="text-muted-foreground hover:text-foreground font-semibold flex items-center justify-center gap-1.5 py-1 px-3 border border-border/60 hover:border-border rounded-xl transition-all w-full h-10 hover:bg-accent/40 dark:hover:bg-accent/10" 
